...
configure_repos() {
  local version=$(get_version)
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║        Multi-Repo Sync - Interactive Configuration         ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
  echo -e "${YELLOW}$version${NC} by ${CYAN}poses98${NC}"
  echo ""
  ...
  read -e -p "[$default_base]: " base_dir
  ...
  while true; do
    ...
    read -e -p "  Key (e.g., 'frontend', 'api', leave empty to finish): " repo_key
    ...
    if [ -d "$base_dir" ]; then
      ...
      read -e -p "  Full path (or directory name under $base_dir): " repo_path
      popd > /dev/null 2>&1
    else
      read -e -p "  Full path (or directory name under $base_dir): " repo_path
    fi
    ...
    if [ -z "$repo_path" ]; then
      echo -e "  ${RED}✗ Path cannot be empty, skipping${NC}"
      echo ""
      continue
    fi
    ...
    # Store the path directly and add to repos list
    new_paths["$repo_key"]="$repo_path"
    new_order+=("$repo_key")
    repos+=("$repo_key:$repo_name")
  ...
  if [ ${#repos[@]} -eq 0 ]; then
    ...
    new_paths["repo1"]="$base_dir/repository-one"
    new_paths["repo2"]="$base_dir/repository-two"
    new_paths["repo3"]="$base_dir/repository-three"
    new_order=("repo1" "repo2" "repo3")
    repos=(
      "repo1:Repository One"
      "repo2:Repository Two"
      "repo3:Repository Three"
    )
  fi
  
  # Now just ask for enable/disable status
  for repo_info in "${repos[@]}"; do
    IFS=':' read -r key name <<< "$repo_info"
    
    echo -e "${BLUE}▸ ${name}${NC}"
    echo -e "  Path: ${CYAN}${new_paths[$key]}${NC}"
...
EOF
  ...
  for key in "${new_order[@]}"; do
    ...
    if [ -d "$path" ]; then
      ...
      echo -e "  ${RED}✗${NC} $key: $path ${RED}(not found)${NC}"
      all_valid=false
    fi
  done
  ...
  if [ "$all_valid" = false ]; then
    echo -e "${YELLOW}⚠️  Some repository paths don't exist yet.${NC}"
...
show_config() {
  ...
  local version=$(get_version)
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║            Current Multi-Repo Sync Configuration               ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo -e "${YELLOW}$version${NC} by ${CYAN}poses98${NC}"
  echo ""
...
show_status() {
  ...
  local version=$(get_version)
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║                  Repositories Status                        ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo -e "${YELLOW}$version${NC} by ${CYAN}poses98${NC}"
  echo ""
  ...
  for key in "${REPO_ORDER[@]}"; do
    ...
    if [ ! -d "$repo" ]; then
      echo -e "  ${RED}✗${NC} $(printf '%-25s' "$key"): ${RED}[not found]${NC}"
      continue
    fi
    ...
    echo -e "  ${GREEN}✓${NC} $(printf '%-25s' "$key"): ${YELLOW}$current_branch${NC} $status"
  done
  
  echo ""
  echo -e "${CYAN}Legend: ${YELLOW}[modified]${NC} = local changes | ${CYAN}↓N${NC} = behind by N commits | ${CYAN}↑N${NC} = ahead by N commits${NC}"
  echo ""
  exit 0
}
...
create_alias() {
  ...
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║                  Alias Installation                          ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo -e "${YELLOW}$version${NC} by ${CYAN}poses98${NC}"
  echo ""
  ...
  read -e -p "[$alias_name]: " custom_alias
  ...
  if grep -q "alias $alias_name=" "$shell_rc" 2>/dev/null; then
    echo -e "${YELLOW}⚠️  Alias '$alias_name' already exists in $shell_rc${NC}"
...
find_branch() {
  ...
  if [ "$branch_count" -gt 1 ]; then
    ...
    echo -e "    ${RED}✗ Multiple branches found matching '$pattern'${NC}" >&2
    echo "" >&2
...
if [ ${#ACTIVE_REPOS[@]} -eq 0 ]; then
  echo -e "${RED}❌ No repositories enabled in configuration${NC}"
  echo -e "${YELLOW}Run: multi-repo-sync --configure${NC}"
  exit 1
fi
...
if [ -z "$BRANCH_PATTERN" ]; then
  echo -e "${RED}❌ Error: Branch pattern not provided${NC}"
...
if [ "$DRY_RUN" = false ]; then
  ...
  if [ ${#REPOS_WITH_CHANGES[@]} -gt 0 ]; then
    echo -e "${YELLOW}⚠️  Uncommitted changes detected in the following repositories:${NC}"
    echo ""
    ...
    case $REPLY in
      1)
        ...
        for key in "${REPOS_WITH_CHANGES[@]}"; do
          ...
          echo -e "  ${GREEN}✓${NC} Stashed $key"
        done
        echo ""
        echo -e "${GREEN}✓ All changes stashed${NC}"
        echo -e "${CYAN}💡 To restore: ${YELLOW}git stash pop${NC} in each repository"
        echo ""
        ;;
      2)
...
echo -e "${BLUE}📦 Step 1: Updating repositories (parallel mode)...${NC}"
...
for key in "${ACTIVE_REPOS[@]}"; do
  ...
  if [ "$result" = "ERROR_MULTIPLE" ]; then
    echo -e "    ${YELLOW}○ Multiple branches found${NC}"
    echo ""
  ...
  if [ "$result" = "ERROR_REMOTE_DELETED" ] || [ "$result" = "ERROR_DELETED_REMOTE" ]; then
    echo -e "    ${YELLOW}○ Branch deleted from remote (only exists locally)${NC}"
  ...
  if [ "$result" = "ERROR_NOT_FOUND" ]; then
    echo -e "    ${YELLOW}○ Branch not found in this repository${NC}"
    FAILED_REPOS+=("$key")
  ...
  case "$result" in
    DRY_RUN_WOULD_CHECKOUT)
      echo -e "    ${CYAN}→ Would checkout and pull${NC}"
      SUCCESSFUL_REPOS+=("$key")
      ;;
    PULLED:*)
      commit_count=$(echo "$result" | cut -d: -f2)
      echo -e "    ${GREEN}✓ Pulled $commit_count commit(s)${NC}"
      SUCCESSFUL_REPOS+=("$key")
      ;;
    UP_TO_DATE)
    ...
    WARNING_LOCAL_CHANGES)
      echo -e "    ${YELLOW}⚠ Checkout ok, but can't pull (local changes)${NC}"
      SUCCESSFUL_REPOS+=("$key")
      ;;
    WARNING_PULL_ISSUES)
      echo -e "    ${YELLOW}⚠ Checkout ok, pull had issues${NC}"
      SUCCESSFUL_REPOS+=("$key")
      ;;
    *)
...
if [ ${#SUCCESSFUL_REPOS[@]} -eq 0 ]; then
  echo ""
  echo -e "${RED}❌ No repositories were updated successfully!${NC}"
  echo ""
  echo -e "${YELLOW}Reasons:${NC}"
  for key in "${FAILED_REPOS[@]}"; do
    echo -e "  ${RED}✗${NC} $key: ${FAILURE_REASONS[$key]}"
  done
  echo ""
  exit 1
fi
...
if [ "$INSTALL_DEPS" = true ] && [ "$DRY_RUN" = false ]; then
  ...
  if [ -n "$lib_repo" ] && [ -d "$lib_repo" ]; then
    echo -e "${BLUE}📚 Step 2: Installing library dependencies...${NC}"
    cd "$lib_repo"
    if [ -f "setup.py" ]; then
      pip install -e . -q
  ...
  echo -e "${CYAN}💡 Would install dependencies (skipped in dry-run mode)${NC}"
  echo ""
fi
...
if [ "$DRY_RUN" = true ]; then
  if [ ${#FAILED_REPOS[@]} -eq 0 ]; then
    echo -e "${CYAN}✓ Dry-run completed successfully!${NC}"
    echo -e "${CYAN}  All repositories would be updated. Run without --dry-run to apply changes.${NC}"
  else
    echo -e "${YELLOW}⚠️  Dry-run completed with some warnings${NC}"
  fi
else
  if [ ${#FAILED_REPOS[@]} -eq 0 ]; then
    echo -e "${GREEN}✓ Synchronization completed successfully!${NC}"
  else
    echo -e "${YELLOW}⚠️  Synchronization completed with some warnings${NC}"
  fi
fi
echo ""

echo -e "${BLUE}📋 Current branch in each repository:${NC}"
for key in "${ACTIVE_REPOS[@]}"; do
  repo="${REPO_PATHS[$key]}"
  ...
  if [[ " ${SUCCESSFUL_REPOS[@]} " =~ " ${key} " ]]; then
    ...
    echo -e "  ${GREEN}✓${NC} $(printf '%-25s' "$key"): ${YELLOW}$current_branch${NC}"
  else
    echo -e "  ${YELLOW}○${NC} $(printf '%-25s' "$key"): ${YELLOW}${FAILURE_REASONS[$key]}${NC}"
  fi
done
...
if [ ${#FAILED_REPOS[@]} -gt 0 ]; then
  echo ""
  echo -e "${YELLOW}⚠️  Some repositories were not updated:${NC}"
  
  has_multiple_branches=false
  for key in "${FAILED_REPOS[@]}"; do
    echo -e "  ${YELLOW}○${NC} $key: ${FAILURE_REASONS[$key]}"
  ...
  if [ "$has_multiple_branches" = true ]; then
    echo ""
    echo -e "${CYAN}💡 Tip:${NC} Use a more specific branch pattern or the full branch name."
